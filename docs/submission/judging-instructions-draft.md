...
The current UI supports four synthetic scenarios, transcript evidence, explicit review/disposition and exact JSON export. The server adapter and persisted dispatch are implemented with current verification recorded in `README.md`. Account/API access was verified read-only. Two accepted real call requests failed with zero transcript. The user confirmed the first did not ring; whether the second rang is unconfirmed. No successful conversation is verified. The displayed 100 credits are not a verified call allowance.
